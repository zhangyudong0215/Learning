...
def to_DataFrame(savepath, filename):
    ...
    location = [i['PRV'] for i in projects]  # 'PRV'   full name, combination of PRV1, 2, 3  所在地区
    ...
    invest_count = [i['INVEST_COUNT'] for i in projects]  # 'INVEST_COUNT' 项目总投资
    proj_type = [i['PROJ_TYPE'] for i in projects]  # 'PROJ_TYPE' 项目种类编号
    proj_type_name = [i['PROJ_TYPE_NAME'] for i in projects]  # 'PROJ_TYPE_NAME' 项目种类名称
    proj_state = [i['PROJ_STATE'] for i in projects]  # 'PROJ_STATE' 所处阶段编号
    proj_state_name = [i['PROJ_STATE_NAME'] for i in projects]  # 'PROJ_STATE_NAME' 所处阶段名称
    return_mode = [i['RETURN_MODE'] for i in projects]  # 'RETURN_MODE' 回报机制编号
    return_mode_name = [i['RETURN_MODE_NAME'] for i in projects]  # 'RETURN_MODE_NAME' 回报机制名称
    survey = [i['PROJ_SURVEY'] for i in projects]  # 'PROJ_SURVEY' 概况
    creating_uname = [i['CREATING_UNAME'] for i in projects]  # 'CREATING_UNAME' 未命名变量1
    ...
    operate_mode = [i['OPERATE_MODE'] for i in projects] # 'OPERATE_MODE' 项目运作方式编号 
    operate_mode_name = [i['OPERATE_MODE_NAME'] for i in projects] # 'OPERATE_MODE_NAME' 项目运作方式名称
    # 增加股东信息
    ...
    guquanbili = {}
    for j in range(20):
        gudongmingcheng[str(j+1)] = [i['gudongmingcheng'+str(j+1)] for i in projects]
        shehuiziben[str(j+1)] = [i['shehuiziben'+str(j+1)] for i in projects]
        chuzie[str(j+1)] = [i['chuzie'+str(j+1)] for i in projects]
        guquanbili[str(j+1)] = [i['guquanbili'+str(j+1)] for i in projects]
