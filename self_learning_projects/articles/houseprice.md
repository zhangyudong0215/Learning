<h1 align = "center">利用数学和数据挖掘方法预测房价</h1>
<div align = "center">数学系 MF1721023 张煜东</div>
<br></br>

> &emsp;&emsp;本文是我用于提交`自然辩证法`课程期末考核的文章. 本文的性质接近于一篇数据分析报告, 主要使用了我在本科和研究生时期学的一些统计学方法及其对应的在数据挖掘领域中实际使用的算法. 由于篇幅限制, 本文中主要就是数据报告的实战部分, 理论部分我打算放到视频中.不过估计在视频里我应该也只会简要介绍几种常见的回归方法, 毕竟要完全展开就太繁琐了. 
> &emsp;&emsp;本文的主题是`Kaggle`这个数据挖掘竞赛网站上的一些入门项目. 这篇文章中我将要完成的是`House Prices`, 一个房价预测项目. 我会通过分析这个房价预测项目介绍数据分析中常用的一些方法, 串联起整个数据分析的基本流程. 
> &emsp;&emsp;另外, 需要提前说明的是文中的代码是`python3`, 运行在`Ubuntu 16.04 LTS`系统中. 而为了插入代码, 图片的方便我是用的是一种叫做`Markdown`的文本标记语言, 有点类似`HTML`(超文本标记语言), 然后在浏览器中打开生成`PDF`, 所以页面周围可能有些奇怪的标注, 不过影响应该不大.

![pic1](./pictures/cartoon1.png)

> &emsp;&emsp;如果我们询问一个买房者他们对于房子特征优先级的排名, 他们可能不会从地下室高度或者离某条主干道的距离这些开始谈论. 但是通过这个项目的分析, 我们明确地发现了买房者确实会对这些问题投入更多的关注, 相较来说他们反而对`卧室数量`, `有没有装修过`关注较少. 简单来说, 我们需要利用数学的方法去寻找主导房价的因素; 因为表面上看起来不重要的特征反而可能对房价的高低起决定性作用. `House Prices`案例提供的数据集有`79`列, 几乎涵盖了一个房子的所有特征, 我们需要利用训练集的数据训练一个预测模型, 然后用于测试集的预测. 接下来正式开始!
...
> &emsp;&emsp;以上便是利用数学里的回归方法和数据挖掘算法预测房价的一个比较完整的例子. 最后得到的结果是其实是三个组合模型的叠加; 而这几个组合模型中又包含了数量更多的简单模型, 因此最后通过加权平均生成的预测结果是一个泛化能力较强的模型.
> &emsp;&emsp;利用数学和数据挖掘方法预测房价的讨论到这里就结束了, 感谢阅读!
