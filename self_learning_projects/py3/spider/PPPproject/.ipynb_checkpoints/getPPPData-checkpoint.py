...
for i in projects:
    if int(i['PROJ_STATE']) is 4:
        ...
        # 项目公司名称
        anchorTag = soup.find('td', attrs={'colspan': '5', 'style': 'text-align: left;width: 30%;'})
        try:
            i['PROJ_COMPANY_NAME'] = str(anchorTag.get_text().split()[0])
        except:
            i['PROJ_COMPANY_NAME'] = None
        # 项目公司成立时间
        anchorTag = soup.find_all('td', attrs={'colspan': '2', 'style': 'text-align: left;width: 30%;'})
        try:
        ...
        if i['gudongcount']:
            for j in range(int(i['gudongcount'])):
                ...
                i['gudongmingcheng' + str(j+1)] = anchorTag[j*5-page_count+1].get_text()
                i['shehuiziben' + str(j+1)] = anchorTag[j*5-page_count+2].get_text()
                i['chuzie' + str(j+1)] = anchorTag[j*5-page_count+3].get_text()
                i['guquanbili' + str(j+1)] = anchorTag[j*5-page_count+4].get_text()
    print('Python Projects ID:%s  完成  累计用时:%d秒' % (str(i['RN']), time.time()-time1))
